...
## Built
- All public pages (home, about, academics, admissions, contact, gallery albums, notices, events, documents, achievements, careers, alumni) in design "Direction B" (chosen 2026-09-24): homepage = notice strip, hero with the logo on a ringed arch + 2 random gallery photos, facts bar, gallery strip (moved up at the owner's request: photos first), academics intro beside 4 stage cards, crimson "why" band, events + notice board, admissions panel. Inner pages share a full-width crimson `PageHeader` band and card lists. Photos are random from every published album: the server picks a random selection (`getRandomPhotos` in `lib/gallery.ts`, new pick each 60 s refresh) and the browser shuffles it on every visit (`components/site/RandomPhotos.tsx`, one shared order per page so photos never repeat; placeholders until shuffled). The Academics page (stages with photos, crimson subjects band, activities mosaic, facilities) uses the same components.
